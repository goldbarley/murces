...
public class Main {
  public static void main(String[] args) {
    ...
    try (InputStream envStream = Main.class.getClassLoader().getResourceAsStream("config.properties")) {
      ...
      er.errorJson("I probably forgot to load .env" + e.toString());
    }
    ...
    while (true) {
      ...
      if (!request.has("type")) {
        er.errorJson("Error Missing A Type Parameter");
        continue;
      }
      ...
      if (type.equals("kill"))
        ...
        if (!(request.has("gameVersion") && request.has("serverVersion") && request.has("serverType")
            && request.has("subType"))) {
          er.errorJson("Missing One Or More Necessary Parameters.");
          continue;
        }
