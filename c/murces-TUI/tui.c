    ...
    menu_items0__ = {.items =
                         {
                             ...
                             'S' | A_UNDERLINE, 't', 'o', 'p', ' ', 's', 'e',
                             'r', 'v', 'e', 'r', 0,
                             /* 2 Configure server */
                             'C' | A_UNDERLINE, 'o', 'n', 'f', 'i', 'g', 'u',
                             'r', 'e', ' ', 's', 'e', 'r', 'v', 'e', 'r', 0,
                             /* 3 Install server */
                             ...
                             /* 5 Create backup */
                             'C',
                             'r', 'e', 'a', 't', 'e', ' ', 'b' | A_UNDERLINE, 'a',
                             'c', 'k', 'u', 'p', 0,
                             /* 6 Browse mods */
                             'B', 'r', 'o', 'w' | A_UNDERLINE, 's',
                             'e', ' ', 'm', 'o', 'd', 's', 0,
                             /* 7 Delete mods */
                             'D' |
                                 A_UNDERLINE,
...
static int (*menus__[7])(struct tui_info *) = {NULL, NULL,  NULL, NULL,
                                               NULL, menu6, NULL};

int mm_init_windows(struct prc_window *desc_win, struct prc_window *content_win,
                    struct prc_window *log_win, struct tui_info *info) {
...
unsigned int mm_get_menu_idx_21__(unsigned int i, unsigned int j) {
  return i * MAX_MENU_ITEM_COUNT + j;
}

int mm_print_menu_item(struct prc_window *win, unsigned int y, unsigned int x,
                       struct menu_items *items, unsigned int n, int idx) {
  if (win == NULL || items == NULL || idx < 0)
    return -1;
  if (!idx)
    return mvwaddchnstr(win->win, y, x, items->items, n);

  return mvwaddchnstr(win->win, y, x,
                      items->items + items->strterms[idx - 1] + 1, n);
}

int mm_insert_menu_text(struct prc_window *win, struct menu_items *items,
  ...
  for (unsigned int idx = 0, y = 0; idx < items->nitems; ++idx, y += 2) {
    ...
    for (int i = 0; curptr < ogsize; ++i) {
      mm_print_menu_item(win, top + i, left, items, bufsize, idx);
      curptr += bufsize;
    }
  }
...
int mm_restore_text0(struct prc_window *desc_win,
                     struct prc_window *content_win) {
  char *dw_desc =
      "MurCes is a Minecraft server configuration"
      " tool. Press <Q> to quit. All navigations are bound to <LEFT>, <RIGHT>,"
      " <UP>, and <DOWN> keys.";

  if (mm_insert_text(desc_win, -1, dw_desc, 4, 4, 2) != 0)
    return -1;
  ...
  if (mm_insert_menu_text(content_win, &menu_items0__, 4, 4, 2) != 0)
    return -1;

  return 0;
}

int mm_insert_text(struct prc_window *win, short pair, char *s, int left,
                   int right, int top) {
  if (win == NULL || s == NULL)
    return -1;
...
int main_menu(struct tui_info *info) {
  ...
  mm_insert_text(
      log_win, -1,
      "VERY VERY BIG BIG LONG LONG TEXT TO TEST THE HOW LONG A MESSAGE IT CAN "
      "WRAP! I AM NOT GOING TO ADD LOGIC TO CHECK FOR WORDS AND SPACES TO WRAP "
      "WORDS PROPERLY SINCE IT IS TO GODDAMN COMPLICATED!",
      4, 4, 2);
  ...
  do {
    ...
    while ((ret = prc_get_first_event(&fevt)) == FN_SUCCESS) {
      ...
      case KEY_UP: {
        ...
        if (mtui_rmhl_menu_item(&menu_items0__, item_selected) != 0)
          return ret;
      ...
      if (chselect) {
        ...
        if (mtui_highlight_menu_item(&menu_items0__, item_selected) != 0)
          return ret;
